...
class GoogleDriveFileSystem(AbstractFileSystem):
    ...
    def __init__(self, root_file_id=None, token="browser",
        ...
        :param use_local_webserver: bool
            Passed to pydata-google-auth's get_user_credentials function. Chooses whether to use
            local web server or to generate a token in the browser.
        :param redirect_uri: None or str
            Redirect URI to pass to PyData, must be associated with client_id and client_secret.
        :param cache_path: None or str
            Path where the .json credentials file generated by PyData is written to. This is 
            also the cache that is searched for when using the 'cache' method.
        :param kwargs:
            Passed to parent
        """
        ...
        self.CUSTOM_READ_WRITE = pydata_google_auth.cache.READ_WRITE
        if self.cache_path != None:
            self.CUSTOM_READ_WRITE._path = self.cache_path
    ...
    def _connect_browser(self):
        try:
            os.remove(self.CUSTOM_READ_WRITE._path)
        except OSError:
            pass
    ...
    def _connect_cache(self):
        if any(self.client_id,self.client_secret,self.redirect_uri):
            if not all(self.client_id,self.client_secret,self.redirect_uri):
                except OSError("client_id/client_secret/redirect_uri must all be provided if any is used.")
                
        return pydata_google_auth.get_user_credentials(
            self.scopes, 
            client_id = self.client_id, client_secret = self.client_secret,
            use_local_webserver = self.use_local_webserver,
            credentials_cache = self.CUSTOM_READ_WRITE,
